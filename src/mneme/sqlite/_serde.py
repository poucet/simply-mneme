...
def _serialize_item(item: StoredContent) -> dict[str, Any]:
    match item:
        ...
        case ToolResult(tool_use_id=tuid, content=content, is_error=err):
            return {"type": "tool_result", "tool_use_id": tuid, "content": content, "is_error": err}
        case _:
...
_DESERIALIZERS = {
    ...
    "tool_result": lambda d: ToolResult(tool_use_id=d["tool_use_id"], content=d["content"], is_error=d.get("is_error", False)),
}
